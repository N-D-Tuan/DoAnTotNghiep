...
BASE_COLUMNS = ["nhiet_do", "do_am", "do_che_phu_may", "ap_suat", "toc_do_gio"]
...
def create_features(df):
    ...
    # --- ĐẶC TRƯNG AN TOÀN (Đo lường biến động của Mây và Độ ẩm) ---
    for col in ["do_am", "do_che_phu_may"]:
        df[f"{col}_std_7d"] = grouped[col].shift(1).rolling(168, min_periods=24).std().reset_index(level=0, drop=True)
        df[f"{col}_std_7d"] = df[f"{col}_std_7d"].fillna(0)
    # ------------------------------------------------------------------------

    df["trang_thai_mua"] = (df["luong_mua"] > 0.1).astype(int)
    df["target_co_mua"] = df["trang_thai_mua"]
    df["target_luong_mua"] = df["luong_mua"]
...
def train_weather_ai():
    ...
    # Nạp thêm 2 cột std vào danh sách đặc trưng huấn luyện
    feature_columns.extend(["do_am_std_7d", "do_che_phu_may_std_7d"])

    df = df.dropna(subset=feature_columns + ["target_co_mua", "target_luong_mua"]).copy()

    train_list = []
    test_list = []
    for cum_san_id, group in df.groupby("cum_san_id"):
        group = group.sort_values("thoi_gian")
    ...
    rain_classifier = RandomForestClassifier(
        ...
        class_weight={0: 1.0, 1: 2.5}, random_state=42, n_jobs=-1 # SỬ DỤNG TRỌNG SỐ TÙY CHỈNH
    )
    ...
    print("\nĐÁNH GIÁ MÔ HÌNH PHÂN LOẠI MƯA TRÊN TẬP TEST MỚI:")
